...
class TrainDataLoader(object):
    def __init__(self, model, seq_dataset, params, out_feature = 19):
    ...
    def generate_path(self):
        ...
        while self.video == 8627 or self.video == 8629 or self.video == 9057 or self.video == 9058:
        ...
        if self.frame == 0:
            ...
            self.template_path = self.seq_dataset[self.video][0][self.frame]
            self.template_box  = self.seq_dataset[self.video][1][self.frame]

            self.ret['template_cropped_resized'] = self.template_crop_resize(self.template_path, self.template_box)
            ...
            self.ret['template_tensor'] = template_tensor.unsqueeze(0).cuda()

            self.ret['kernel_reg'], self.ret['kernel_cls'] = self.model.learn(self.ret['template_tensor'])


            '''detection_frame'''
            self.detection_path = self.seq_dataset[self.video][0][self.frame+1]
            self.detection_box  = self.seq_dataset[self.video][1][self.frame+1]

            self.ret['detection_cropped_resized'], self.ret['target_in_resized_detection_xywh'] = self.detection_crop_resize(self.detection_path, self.detection_box)
    ...
    def template_crop_resize(self, template_path, template_box):
    ...
    def detection_crop_resize(self, detection_path, detection_box):
        detection_img = Image.open(detection_path)
